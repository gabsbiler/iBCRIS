...
const fetchData = async () => {
  try{  
    const response = await axiosIns.get(`/api/dashboard/member/count/42_2?barangay=${localStorage.getItem('selectedBarangay')}`)
...
<template>
  <VCard title="Continous Medical Care">
    <VCardText>
      <VTable style=" height: 406px;margin-top: -3px;">
        <thead>
          <tr>
            <th>Continous Medical Care</th>
            <th>Count</th>
          </tr>
        </thead>
        <tbody>
